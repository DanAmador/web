...
function toggleKey(e) {
  ...
  var n = availableKeys[key];

  if (invisibleColumns.has(n)) {
    invisibleColumns.delete(n);

    var _iterator = _createForOfIteratorHelper(document.getElementsByClassName(n)),
        _step;
    ...
    var _iterator2 = _createForOfIteratorHelper(document.getElementsByClassName(n)),
        _step2;
...
function BuildTable() {
  ...
  var keys = Object.keys(availableKeys);
  var counter = 0;
  ...
  for (var _i = 0, _keys = keys; _i < _keys.length; _i++) {
    ...
    var className = availableKeys[item];
  ...
  for (var _i2 = 0, _dataset = dataset; _i2 < _dataset.length; _i2++) {
    var d = _dataset[_i2];
    ...
    try {
      for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
        var k = _step3.value;
        var _className = availableKeys[k];
        var td = document.createElement('td');
        td.appendChild(document.createTextNode(d[k]));
        td.classList.add(_className);
        tRow.appendChild(td);
      }
    } catch (err) {
      _iterator3.e(err);
...
var bttns = document.getElementsByClassName("Toggler");
addKey("id");
addKey("name");
